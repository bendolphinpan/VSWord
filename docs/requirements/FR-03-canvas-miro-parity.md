...
VSWord Canvas 与 Miro 最大不同：**节点可以是真实文件引用**。需要监听文件系统变化。
...
| 在 Canvas 中删除文件卡片 | **只删 Canvas 节点，不删真实文件**（默认）；右键有"同时删除文件"选项 |
| 在 Canvas 中"新建文件卡片" | 通过 IFileService 创建真实文件 + 立即建 Canvas 节点 |
| 拖入外部文件到 Canvas | 默认建"引用"节点（不复制文件）；按 Ctrl 拖入 → 复制到当前文件夹 |
...
- 同一 workspace 文件多次被拖入：仅创建一个新节点，但不阻止重复（用户可能想要"同一文件在 Canvas 不同位置"）。
- 拖入 workspace 外文件：弹框询问"复制到 workspace" / "保留外部链接（绝对路径）"。

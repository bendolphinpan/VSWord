...
| **RD-9** | Phase 5 Mindmap 剩余 | master §3 | P2 · **阶段收口 ✅**（9.2/9.4 延后） | 功能 | — | 否 |
...
| RD-1.3 | **RD-1-lite**：首交互/加载中可编辑 + 进度 + dirty 正确；首块 48k | ✅ 代码（手测可选） |
...
### RD-4 — 模式正交 UI 收尾对账 · P2 · ✅（2026-07-13）

**现状**：T-3.12.3.a/b + T-3.13.2/3 已落地。**权威语义**以 T-3.13.2 为准（阅读下 **保留** 二级 substyle 三档），003 v2「阅读隐藏二级」作废。

| 子项 | 内容 | 状态 |
|------|------|------|
| RD-4.1 | 对照 003 §7 / AC，逐条对账 | ✅ `docs/decisions/0005-rd4-mode-orthogonality.md` |
| RD-4.2 | 单测：viewModes 37 + modeSwitch 8 绿；`viewModeMatrix` 7 pending（旧双 toggle，不阻塞） | ✅ |
| RD-4.3 | 关 U-4；视觉精修归 UI 布局阶段 | ✅ |
...
### RD-9 — Phase 5 Mindmap 剩余 · P2 · 阶段收口（2026-07-13）

详见 `docs/decisions/0007-rd9-mindmap-remaining.md`。

| 子项 | 内容 | 状态 |
|------|------|------|
| RD-9.1 | Markdown ↔ `.mm` 互转 | ✅ |
| RD-9.1b | ATX 标题 + 列表混排导入 | ✅ |
| RD-9.1c | `.mm` → Markdown 大纲导出命令 | ✅ `mindmapToMarkdownOutline` |
| RD-9.2 | richcontent 富文本可视化编辑 | ⏸ 延后 |
| RD-9.3 / 9.3b | 1k fixture + **10k 合成 parse 实测** | ✅ ~30ms parse · 非 mind-elixir 渲染 |

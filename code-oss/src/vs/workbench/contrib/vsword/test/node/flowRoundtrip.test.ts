...
// T-3.5b-flowseq.3c · Flow round-trip 保真度测试
//
// 每类 fixture：
//   1) 用 flow-render-worker.mjs（子进程 fork）在 jsdom + raphael 里跑 flowchart.js 渲染
//   2) normalize 后与 .golden.svg 比对
//   3) 若 golden 缺失，则用当前 normalized 作为 golden（首次生成兜底）
//
// 与 mermaidRoundtrip.test.ts 一样通过 fork 子进程完成渲染，mocha 主进程只做 diff。
...
import { FLOW_FIXTURES } from '../fixtures/flow/_index.mjs';
...
const FIXTURE_DIR = path.resolve(CODE_OSS, 'src/vs/workbench/contrib/vsword/test/fixtures/flow');
...
interface WorkerPayload { flowchartVersion: string; raphaelVersion: string; builder: string; iterations: number; total: number; okCount: number; failCount: number; results: WorkerResult[]; }

let cachedPayload: WorkerPayload | null = null;

function runWorkerOnce(): WorkerPayload {
	if (cachedPayload) { return cachedPayload; }
	const tmpOut = path.join(REPO_ROOT, `code-oss/test/reports/_flow-roundtrip-${Date.now()}.json`);
	...
	if (res.status !== 0) { throw new Error(`flow-render-worker exited with status ${res.status}`); }
	if (!fs.existsSync(tmpOut)) { throw new Error(`flow-render-worker did not produce ${tmpOut}`); }
	cachedPayload = JSON.parse(fs.readFileSync(tmpOut, 'utf8'));
	try { fs.unlinkSync(tmpOut); } catch { /* keep */ }
	return cachedPayload!;
}

suite('T-3.5b-flowseq.3c · Flow round-trip · golden svg diff', function () {
	...
	test(`worker: ${FLOW_FIXTURES.length}/${FLOW_FIXTURES.length} render 成功`, () => {
		assert.strictEqual(payload.total, FLOW_FIXTURES.length, `expected ${FLOW_FIXTURES.length} fixtures, got ${payload.total}`);
		assert.strictEqual(payload.failCount, 0, `render failed for ${payload.results.filter(r => !r.ok).map(r => r.id).join(', ')}`);
	});

	for (const fx of FLOW_FIXTURES as Array<{ id: string; tier: string }>) {
		test(`[flow/${fx.tier}] ${fx.id} · normalized svg == golden`, () => {

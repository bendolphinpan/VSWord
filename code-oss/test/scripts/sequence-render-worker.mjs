#!/usr/bin/env node
/*---------------------------------------------------------------------------------------------
 *  T-3.5b-flowseq.3c · Sequence render worker（helper 子进程）
 *
 *  被 gate-f / sequenceRoundtrip.test / sequence-selfcheck 共用。
 *  在 jsdom + raphael + underscore 里跑 @rokt33r/js-sequence-diagrams，
 *  渲染 fixture 集合，输出 JSON 到 --out。
 *
 *  用法：
 *    node sequence-render-worker.mjs \
 *      --ids seq-basic,seq-actors,... \
 *      [--iterations 1] \
 *      --out <json-path>
 *
 *  与 flow-render-worker.mjs 差别：
 *    • require 顺序敏感：Raphael + underscore 必须先挂到 globalThis 再 require
 *      @rokt33r/js-sequence-diagrams（模块顶层读全局）
 *    • drawSVG 走 theme=simple；hand 主题依赖 handjs，暂不覆盖
 *--------------------------------------------------------------------------------------------*/
...
function pickBuilder() {
	if (fs.existsSync(path.join(BUILDER_A, 'node_modules', '@rokt33r/js-sequence-diagrams/package.json'))) { return BUILDER_A; }
	if (fs.existsSync(path.join(BUILDER_B, 'node_modules', '@rokt33r/js-sequence-diagrams/package.json'))) { return BUILDER_B; }
	throw new Error('@rokt33r/js-sequence-diagrams not found under any .tmp/milkdown-prod-builder path');
}
...
async function main() {
	...
	const seqPkg = JSON.parse(fs.readFileSync(path.join(builder, 'node_modules/@rokt33r/js-sequence-diagrams/package.json'), 'utf8'));
	const raphaelPkg = JSON.parse(fs.readFileSync(path.join(builder, 'node_modules/raphael/package.json'), 'utf8'));
	const underscorePkg = JSON.parse(fs.readFileSync(path.join(builder, 'node_modules/underscore/package.json'), 'utf8'));

	const { normalizeFlowSeqSvg } = await import(pathToFileURL(path.join(VSWORD, 'test/utils/flowseq-svg-normalize.mjs')).href);
	const { SEQUENCE_FIXTURES } = await import(pathToFileURL(path.join(VSWORD, 'test/fixtures/sequence/_index.mjs')).href);
	...
	// —— 装 raphael / underscore / sequence（顺序敏感）——
	const Raphael = requireFromBuilder('raphael');
	const underscore = requireFromBuilder('underscore');
	globalThis.Raphael = Raphael;
	globalThis._ = underscore;
	try { dom.window.Raphael = Raphael; } catch { /* ok */ }
	try { dom.window._ = underscore; } catch { /* ok */ }
	const seq = requireFromBuilder('@rokt33r/js-sequence-diagrams');
	...
	const targets = SEQUENCE_FIXTURES.filter(f => !wantedIds || wantedIds.has(f.id));
	const results = [];
	for (const f of targets) {
		...
		for (let it = 0; it < iterations; it++) {
			...
			const t0 = realTimer();
			try {
				const diagram = Diagram.parse(f.src);
	...
	const payload = {
		sequenceVersion: seqPkg.version,
		raphaelVersion: raphaelPkg.version,
		underscoreVersion: underscorePkg.version,
		builder,
		iterations,
...
main().then(() => {
	...
	process.stderr.write('[sequence-render-worker] fatal: ' + (err && err.stack || err) + '\n');
	process.exit(2);
});

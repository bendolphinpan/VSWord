#!/usr/bin/env node
// Ad-hoc runner for T-3.7b.a · vswordViewModeService.test.ts
// 单文件测试，无 milkdown 依赖，esbuild 直接 bundle + mocha 跑。
...
const TEST_FILE = path.join(
	CODE_OSS,
	'src/vs/workbench/contrib/vsword/test/node/vswordViewModeService.test.ts',
);
...
async function main() {
	console.log('[view-mode-test] start');
	if (!fs.existsSync(TEST_FILE)) {
		throw new Error(`missing test: ${TEST_FILE}`);
	}
	const esbuild = await loadEsbuild();
	console.log('[view-mode-test] esbuild loaded');
	const outdir = fs.mkdtempSync(path.join(CODE_OSS, '.tmp-viewmode-'));
	...
	console.log('[view-mode-test] bundled at', outfile);
	const mochaCli = path.join(
		CODE_OSS,
		'node_modules/.bin/mocha' + (process.platform === 'win32' ? '.cmd' : ''),
	);
	console.log('[view-mode-test] mochaCli=', mochaCli, 'exists=', fs.existsSync(mochaCli));
	const res = cp.spawnSync(mochaCli, ['--ui', 'tdd', '--reporter', 'spec', outfile], {
		encoding: 'utf8',
		env: process.env,
		shell: process.platform === 'win32',
	});
	console.log('[view-mode-test] mocha exit code=', res.status, 'signal=', res.signal, 'err=', res.error && res.error.message);

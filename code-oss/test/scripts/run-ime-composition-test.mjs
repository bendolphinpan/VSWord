#!/usr/bin/env node
/*---------------------------------------------------------------------------------------------
 *  T-3.9.2.c · IME composition 单测 ad-hoc runner
 *
 *  职责：把 imeComposition.test.ts bundle 起来，用 mocha (tdd) 跑。
 *  jsdom 走 gate-e 的 loader-stub + alias plugin 模式（避免 esbuild bundle jsdom
 *  内部的 native shim；由运行期 require 从 .tmp/milkdown-prod-builder 拿）。
 *
 *  用法：node code-oss/test/scripts/run-ime-composition-test.mjs
 *--------------------------------------------------------------------------------------------*/
...
const TEST_FILE = path.join(
	CODE_OSS,
	'src/vs/workbench/contrib/vsword/test/node/imeComposition.test.ts',
);
...
async function main() {
	console.log('[ime-composition-test] start');
	if (!fs.existsSync(TEST_FILE)) {
		throw new Error(`missing test: ${TEST_FILE}`);
	}
	const esbuild = await loadEsbuild();
	const outdir = fs.mkdtempSync(path.join(CODE_OSS, '.tmp-ime-composition-'));
	...
	console.log('[ime-composition-test] bundled at', outfile);
	...
	console.log('[ime-composition-test] mocha exit=', res.status, 'elapsed=', elapsed, 'ms');

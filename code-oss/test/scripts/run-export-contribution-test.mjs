#!/usr/bin/env node
// Ad-hoc runner for T-3.8b.1.b · exportContribution.test.ts
// 单文件测试，esbuild bundle + mocha 跑。对齐 run-view-mode-actions-test.mjs。
...
const TEST_FILE = path.join(
	CODE_OSS,
	'src/vs/workbench/contrib/vsword/test/node/exportContribution.test.ts',
);
...
async function main() {
	console.log('[export-contribution-test] start');
	if (!fs.existsSync(TEST_FILE)) {
		throw new Error(`missing test: ${TEST_FILE}`);
	}
	const esbuild = await loadEsbuild();
	console.log('[export-contribution-test] esbuild loaded');
	const outdir = fs.mkdtempSync(path.join(CODE_OSS, '.tmp-export-contribution-'));
	...
	console.log('[export-contribution-test] bundled at', outfile);
	const mochaCli = path.join(
		CODE_OSS,
		'node_modules/.bin/mocha' + (process.platform === 'win32' ? '.cmd' : ''),
	);
	console.log('[export-contribution-test] mochaCli=', mochaCli, 'exists=', fs.existsSync(mochaCli));
	const res = cp.spawnSync(mochaCli, ['--ui', 'tdd', '--reporter', 'spec', outfile], {
		encoding: 'utf8',
		env: process.env,
		shell: process.platform === 'win32',
	});
	console.log('[export-contribution-test] mocha exit code=', res.status, 'signal=', res.signal, 'err=', res.error && res.error.message);

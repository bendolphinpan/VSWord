#!/usr/bin/env node
/*---------------------------------------------------------------------------------------------
 *  T-3.12.2.a · table-chrome NodeView 单测 ad-hoc runner。
 *
 *  与 run-toc-interaction-test.mjs 同构：
 *    - `./foo.mjs` → `./foo.template.js` 别名重写
 *    - jsdom / @milkdown/* / mocha 从 builder node_modules 解析（NODE_PATH + external）
 *
 *  用法：node code-oss/test/scripts/run-table-chrome-test.mjs
 *--------------------------------------------------------------------------------------------*/
...
const ENTRY = path.join(
	CODE_OSS,
	'src/vs/workbench/contrib/vsword/test/node/tableChrome.test.ts',
);
...
function makeAliasPlugin() {
	...
	return {
		name: 'vsword-table-chrome-alias',
		setup(build) {
...
async function bundleAndRun(esbuild) {
	const outdir = fs.mkdtempSync(path.join(CODE_OSS, `.tmp-table-chrome-`));
	const outfile = path.join(outdir, 'test.mjs');
	console.log(`[table-chrome-test] bundling ${ENTRY} …`);
	await esbuild.build({
		entryPoints: [ENTRY],
		bundle: true,
		format: 'esm',
	...
	console.log(`[table-chrome-test] bundled at`, outfile);

	const mochaCli = path.join(
		CODE_OSS,
		'node_modules/.bin/mocha' + (process.platform === 'win32' ? '.cmd' : ''),
	);
	console.log(`[table-chrome-test] mochaCli=`, mochaCli, 'exists=', fs.existsSync(mochaCli));
	...
	console.log(`[table-chrome-test] mocha exit=`, res.status, 'signal=', res.signal);
...
async function main() {
	console.log('[table-chrome-test] start');

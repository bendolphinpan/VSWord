...
const ENTRY = path.join(CODE_OSS, 'src/vs/workbench/contrib/vsword/test/node/typography.test.ts');

const esbuild = (await import(pathToFileURL(path.join(BUILDER, 'node_modules/esbuild/lib/main.js')).href)).default;
const outdir = fs.mkdtempSync(path.join(CODE_OSS, '.tmp-typography-'));

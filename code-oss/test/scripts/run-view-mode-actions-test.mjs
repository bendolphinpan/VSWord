#!/usr/bin/env node
// Ad-hoc runner for T-3.7b.b · vswordViewModeActions.test.ts
// 单文件测试，esbuild 直接 bundle + mocha 跑。风格与 run-view-mode-test.mjs 对齐。
...
const TEST_FILE = path.join(
	CODE_OSS,
	'src/vs/workbench/contrib/vsword/test/node/vswordViewModeActions.test.ts',
);
...
async function main() {
	console.log('[view-mode-actions-test] start');
	if (!fs.existsSync(TEST_FILE)) {
		throw new Error(`missing test: ${TEST_FILE}`);
	}
	const esbuild = await loadEsbuild();
	console.log('[view-mode-actions-test] esbuild loaded');
	const outdir = fs.mkdtempSync(path.join(CODE_OSS, '.tmp-viewmode-actions-'));
	...
	console.log('[view-mode-actions-test] bundled at', outfile);
	const mochaCli = path.join(
		CODE_OSS,
		'node_modules/.bin/mocha' + (process.platform === 'win32' ? '.cmd' : ''),
	);
	console.log('[view-mode-actions-test] mochaCli=', mochaCli, 'exists=', fs.existsSync(mochaCli));
	const res = cp.spawnSync(mochaCli, ['--ui', 'tdd', '--reporter', 'spec', outfile], {
		encoding: 'utf8',
		env: process.env,
		shell: process.platform === 'win32',
	});
	console.log('[view-mode-actions-test] mocha exit code=', res.status, 'signal=', res.signal, 'err=', res.error && res.error.message);

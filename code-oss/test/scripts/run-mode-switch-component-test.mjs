#!/usr/bin/env node
/*---------------------------------------------------------------------------------------------
 *  T-3.7b.d · ModeSwitchComponent 单元测试 ad-hoc runner
 *
 *  职责：把 modeSwitchComponent.test.ts bundle 起来，用 mocha (tdd) 跑。
 *  参考 run-flow-sequence-theme-bridge.mjs 的 alias + jsdom loader pattern
 *  （因为 mode-switch.template.js import 了 ./ui-component.mjs，需要 alias
 *  改写到 .template.js；同时 test 用 jsdom）。
 *
 *  用法：node code-oss/test/scripts/run-mode-switch-component-test.mjs
 *--------------------------------------------------------------------------------------------*/
...
const TEST_FILE = path.join(
	CODE_OSS,
	'src/vs/workbench/contrib/vsword/test/node/modeSwitchComponent.test.ts',
);
	...
	return {
		...
		setup(build) {
			...
			build.onResolve({ filter: /.*/ }, args => {
				...
				// ./foo.mjs → ./foo.template.js（gate-e / theme-bridge runner 同款 pattern）
...
async function main() {
	console.log('[mode-switch-component-test] start');
	if (!fs.existsSync(TEST_FILE)) {
		throw new Error(`missing test: ${TEST_FILE}`);
	}
	const esbuild = await loadEsbuild();
	console.log('[mode-switch-component-test] esbuild loaded');
	const outdir = fs.mkdtempSync(path.join(CODE_OSS, '.tmp-mode-switch-component-'));
	...
	console.log('[mode-switch-component-test] bundled at', outfile);
	const mochaCli = path.join(
		CODE_OSS,
		'node_modules/.bin/mocha' + (process.platform === 'win32' ? '.cmd' : ''),
	);
	console.log('[mode-switch-component-test] mochaCli=', mochaCli, 'exists=', fs.existsSync(mochaCli));
	...
	console.log('[mode-switch-component-test] mocha exit code=', res.status, 'signal=', res.signal, 'err=', res.error && res.error.message);

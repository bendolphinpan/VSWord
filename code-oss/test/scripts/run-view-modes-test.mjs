#!/usr/bin/env node
// Ad-hoc runner for T-3.10 · viewModes.test.ts (mode-controller regression).
// 复用 run-flow-sequence-theme-bridge.mjs 的 jsdom alias pattern。
...
const TEST_FILE = path.join(
	CODE_OSS,
	'src/vs/workbench/contrib/vsword/test/node/viewModes.test.ts',
);
...
async function main() {
	...
	const outdir = fs.mkdtempSync(path.join(CODE_OSS, '.tmp-viewmodes-'));

#!/usr/bin/env node
/*---------------------------------------------------------------------------------------------
 *  T-3.5b-flowseq.3c · Sequence selfcheck（same-runtime × 3 遍稳定性）
 *--------------------------------------------------------------------------------------------*/
...
function main() {
	...
	const tmpOut = path.join(REPORTS_DIR, `_seq-selfcheck-run-${Date.now()}.json`);

	process.stderr.write('[seq-selfcheck] fork sequence-render-worker --iterations 3 ...\n');
	...
	if (res.status !== 0) { process.stderr.write(`[seq-selfcheck] worker exited with status ${res.status}\n`); process.exit(2); }
	if (!fs.existsSync(tmpOut)) { process.stderr.write('[seq-selfcheck] worker did not produce output json\n'); process.exit(2); }
	...
	for (const r of payload.results) {
		...
		const equal01 = svgs[0] === svgs[1]; const equal12 = svgs[1] === svgs[2];
	...
	const mdPath = path.join(REPORTS_DIR, `seq-selfcheck-${stamp}.md`);
	const md = [];
	md.push(`# Sequence Self-check · ${new Date().toISOString()}`);
	md.push('');
	md.push(`- @rokt33r/js-sequence-diagrams version: ${payload.sequenceVersion}`);
	md.push(`- raphael version: ${payload.raphaelVersion}`);
	md.push(`- underscore version: ${payload.underscoreVersion}`);
	...
	process.stderr.write(`[seq-selfcheck] 报告 -> ${mdPath}\n`);
	process.stderr.write(`[seq-selfcheck] stable=${rows.filter(r => r.stable).length}/${rows.length}, elapsed=${elapsed}ms\n`);
...
try { main(); } catch (err) {
	process.stderr.write('[seq-selfcheck] fatal: ' + (err && err.stack || err) + '\n');
	process.exit(2);
}
